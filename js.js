 ...
 * @property {Object} nextStepHeadPoint Мария. Хранит будущее положение головы с учетом выхода с противоположной стороны.
 */
const snake = { 
  body: null,
  direction: null,
  lastStepDirection: null,
  nextStepHeadPoint: null,
  ...
  makeStep() {
    ...
    //Мария. Будем брать след. положение из переменной с учетом выхода через противоположную сторону.
    this.body.unshift(this.nextStepHeadPoint);
 ...
 * @property {int} countScore Счетчик игры.
 */
const game = {
  config,
  map,
  ...
  reset() {
    ...
    //Мария После инициализации змейки обновляем счет
    this.renderScoreOnBoard();
  ...
  tickHandler() {
    ...
    // Мария. Если следующий шаг выходит за пределы поля
    this.snake.nextStepHeadPoint = this.snake.getNextStepHeadPoint();
    //const nextHeadPoint = this.snake.getNextStepHeadPoint();
    if (this.goOnSide()) { // если выходим за пределы поля
      this.setNewPointFromOppositeSide(); 
    }
  
    
    // Если следующий шаг будет на еду, то заходим в if.
    //if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      if (this.food.isOnPoint(this.snake.nextStepHeadPoint)) {
      // Прибавляем к змейке ячейку.
      this.snake.growUp();
      //Мария. Выводим новое значение очков
      this.renderScoreOnBoard();
  ...
  /**
   * Мария Проверяет, выйдет ли змейка за пределы поля.
   * @param {Object} nextHeadPoint Следующая точка.
   * @returns {boolean}  true, если змейка выходит за пределы поля, иначе false.
   */
  goOnSide() {
    return this.snake.nextStepHeadPoint.x > this.config.getColsCount()-1 ||
    this.snake.nextStepHeadPoint.y > this.config.getRowsCount()-1 ||
    this.snake.nextStepHeadPoint.x < 0 ||
    this.snake.nextStepHeadPoint.y < 0;
  },

  /**
   * 
   * @param {{x: int, y: int}} nextHeadPoint Объект будущей точки без учета переноса на новую сторону
   */
  setNewPointFromOppositeSide() {

   if (this.snake.nextStepHeadPoint.x > this.config.getColsCount()) { // направо
     this.snake.nextStepHeadPoint = {x: 0, y: this.snake.nextStepHeadPoint.y}; 
     return;
   }
   if (this.snake.nextStepHeadPoint.x < 0) { // налево
    this.snake.nextStepHeadPoint = {x: this.config.getColsCount()-1, y: this.snake.nextStepHeadPoint.y}; 
    return;
  }
  if (this.snake.nextStepHeadPoint.y < 0) { // наверх
    this.snake.nextStepHeadPoint = {x: this.snake.nextStepHeadPoint.x, y: this.config.getRowsCount()-1}; 
    return;
  }
  if (this.snake.nextStepHeadPoint.y > this.config.getRowsCount()) { // вниз
    this.snake.nextStepHeadPoint = {x: this.snake.nextStepHeadPoint.x, y: 0}; 
    return;
  }
  ...
  /**
   * Выводит на экран количество очков (длину хвоста змейки)
   */
  renderScoreOnBoard() {
    document.getElementById('score').textContent = this.snake.getBody().length-1;
  },
